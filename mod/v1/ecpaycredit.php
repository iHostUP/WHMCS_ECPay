...
function ecpaycredit_link($params) {
	...
	$MerchantTradeNo = 'ECIC'.$invoiceid.'T'.time();
	...
	$post_value= array(
		...
		'ReturnURL' => $systemurl.'/modules/gateways/callback/ecpaycredit.php',
		'ChoosePayment' => 'Credit',
		'NeedExtraPaidInfo' => $gatewayNeedExtraPaidInfo,
	);
	
	$CheckMacValue = curl_ecpaycredit_post($CheckMacValueurl,$post_value);
	...
	<input type=hidden name="ReturnURL" value="'.$systemurl.'/modules/gateways/callback/ecpaycredit.php">			<!--付款成功後POST的地址-->
...
function curl_ecpaycredit_post($url,$post)
{
	 $ch = curl_init();
