...
function ecpaycvs_link($params) {
	...
	$post_value= array(
		...
		'MerchantTradeNo' => 'ECICVS'.$invoiceid,
		'MerchantTradeDate' => $nowdate,
		'PaymentType' => 'aio',
		'TotalAmount' => $TotalAmount,
		'TradeDesc' => $gatewayTradeDesc,
		'ItemName' => $description,
		'ReturnURL' => $systemurl.'/modules/gateways/callback/ecpaycvs.php',
		'ChoosePayment' => 'CVS',
		...
		'PaymentInfoURL' => $systemurl.'/modules/gateways/ecpay/ecpaygetcvs.php',
		'ClientRedirectURL' => $systemurl.'/viewinvoice.php?id='.$invoiceid,
	);
	
	$CheckMacValue = curl_ecpaycvs_post($CheckMacValueurl,$post_value);
	
	# 跳轉頁面
	$sql = "SELECT * FROM mod_ecpay where MerchantTradeNo = 'ECICVS$invoiceid'";
	$result = mysql_query($sql);
	$row = @mysql_fetch_row($result);
	if($row==null){
		$code = '<form action="'.$posturl.'" method="post">
		<input type=hidden name="MerchantID" value="'.$gatewayMerchantID.'">											<!--廠商ID-->
		<input type=hidden name="MerchantTradeNo" value="ECICVS'.$invoiceid.'">												<!--廠商交易編號-->
		...
		<input type=hidden name="ReturnURL" value="'.$systemurl.'/modules/gateways/callback/ecpaycvs.php">	<!--付款成功後POST的地址-->
		...
		<input type=hidden name="PaymentInfoURL" value="'.$systemurl.'/modules/gateways/ecpay/ecpaygetcvs.php">		<!--回傳給WHMCS取號通知-->
...
function curl_ecpaycvs_post($url,$post)
{
	 $ch = curl_init();
